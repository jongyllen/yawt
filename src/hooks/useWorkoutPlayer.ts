...
export function useWorkoutPlayer({
     ...
     */
    const handleNext = useCallback(async (shouldPause: boolean = false) => {
        ...
        hasCompletedRef.current = false;
    }, [workout, currentBlockIndex, currentStepIndex, currentRound, clearTimer, getNextExerciseName, saveState]);
     ...
     */
    const handlePauseChange = useCallback(async (paused: boolean) => {
        if (!persistedStateRef.current) return;
    ...
    }, [timeLeft, isPaused, isFinished, calculateTimeRemaining, getNextExerciseName, handleNext, clearTimer]);
    ...
    }, [isPaused, isFinished, syncFromPersistedState, getNextExerciseName, handleNext, calculateTimeRemaining, scheduleTimerNotification]);
    ...
    }, [workout, currentBlockIndex, currentStepIndex, currentRound, timeLeft, isPaused, isFinished, getProgress]);
