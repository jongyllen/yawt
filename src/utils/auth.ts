...
export async function signInWithApple(): Promise<CloudUser | null> {
    ...
    try {
        ...
        if (e.code === 'ERR_CANCELED' || e.code === '1001') { // 1001 is Apple's cancel code
            return null;
        }
        throw e;
    }
...
export async function getAppleAuthStatus(): Promise<AppleAuthentication.AppleAuthenticationCredentialState> {
    if (Platform.OS !== 'ios') return AppleAuthentication.AppleAuthenticationCredentialState.NOT_FOUND;
...
export async function getStoredUserId(): Promise<string | null> {
    return (await SecureStore.getItemAsync(APPLE_USER_ID_KEY)) || (await SecureStore.getItemAsync(GOOGLE_USER_ID_KEY));
}

export async function getStoredUserName(): Promise<string | null> {
    return (await SecureStore.getItemAsync(APPLE_USER_NAME_KEY)) || (await SecureStore.getItemAsync(GOOGLE_USER_NAME_KEY));
}
