...
export async function initHealthKit(): Promise<{ success: boolean; error?: string }> {
  ...
  if (!Device.isDevice) {
    console.warn('[HealthKit] HealthKit is not available on simulator.');
    return { success: false, error: 'HealthKit is not available on simulator.' };
  }
  ...
  return new Promise((resolve) => {
    ...
    if (!NativeAppleHealthKit) {
      if (Device.isDevice) {
        console.warn(
          '[HealthKit] Native module is missing. Ensure you are using a development build with the HealthKit plugin.'
        );
      }
...
export async function saveWorkoutToHealth(options: {
  ...
  // Final safety check for the function existence
  if (typeof AppleHealthKit.saveWorkout !== 'function') {
    console.warn('[HealthKit] saveWorkout function not found. Module might not be fully loaded.');
    return false;
  }
