...
async function getStorageInstance() {
  ...
  if (Platform.OS === 'android') {
    try {
      ...
      if (accessToken) {
        CloudStorage.setAccessory(
          new GoogleDriveAccessory({
            accessToken,
            scope: 'appDataFolder',
          })
        );
...
export async function uploadToCloud(): Promise<void> {
  try {
    ...
    console.log(
      `Successfully uploaded backup to ${Platform.OS === 'ios' ? 'iCloud' : 'Google Drive'} at`,
      backup.updatedAt
    );
  } catch (e) {
...
export async function downloadFromCloud(): Promise<boolean> {
  try {
    ...
    console.log(
      `Successfully downloaded and restored backup from ${Platform.OS === 'ios' ? 'iCloud' : 'Google Drive'}`
    );
    return true;
  } catch (e) {
...
export async function syncIfNewer(): Promise<void> {
  try {
    ...
    if (
      !lastLocalSync ||
      (cloudBackup.updatedAt && new Date(cloudBackup.updatedAt) > new Date(lastLocalSync))
    ) {
...
async function applyRestoreSettings(s: any) {
  ...
  if (s.haptics_enabled !== undefined)
    await AsyncStorage.setItem('haptics_enabled', s.haptics_enabled.toString());
  if (s.countdown_beeps_enabled !== undefined)
    await AsyncStorage.setItem('countdown_beeps_enabled', s.countdown_beeps_enabled.toString());
}
