...
export const initDatabase = async () => {
  ...
  initPromise = (async () => {
    ...
    // Run migrations (columns individually with try/catch)
...
export const getWorkoutLogs = async (db: SQLite.SQLiteDatabase): Promise<WorkoutLog[]> => {
  const rows = await db.getAllAsync<{ data: string }>(
    'SELECT data FROM workout_logs ORDER BY date DESC'
  );
  return rows.map((row) => JSON.parse(row.data));
};

export const getWorkoutLogsByProgram = async (
  db: SQLite.SQLiteDatabase,
  programId: string
): Promise<WorkoutLog[]> => {
  const rows = await db.getAllAsync<{ data: string }>(
    'SELECT data FROM workout_logs WHERE program_id = ? ORDER BY date DESC',
    programId
  );
  return rows.map((row) => JSON.parse(row.data));
};
...
export const getActivePrograms = async (db: SQLite.SQLiteDatabase): Promise<ActiveProgram[]> => {
  try {
    const rows = await db.getAllAsync<{ data: string }>(
      'SELECT data FROM active_programs WHERE status = "active" ORDER BY updated_at DESC'
    );
    return rows
      .map((row) => {
        try {
...
export const getActiveProgramByProgramId = async (
  db: SQLite.SQLiteDatabase,
  programId: string
): Promise<ActiveProgram | null> => {
  const row = await db.getFirstAsync<{ data: string }>(
    'SELECT data FROM active_programs WHERE program_id = ? AND status = "active"',
    programId
  );
  return row ? JSON.parse(row.data) : null;
};
...
export const getNextWorkoutForProgram = async (
  db: SQLite.SQLiteDatabase,
  program: Program
): Promise<{ workout: Workout; weekNumber?: number; dayNumber?: number } | null> => {
